...
const DEFAULT_WALLET = { tokens: 0, owned: ['neon'], equipped: 'neon', asteroidUpgrades: { extraLife: 0, turnSpeed: 0, autoTurret: 0 } };
...
app.post('/api/asteroid/upgrades/purchase', async (req, res) => {
  ...
  try {
    const result = await withWriteLock(() => {
      const data = loadData();
      const wallet = data[user].wallet;
      const level = wallet.asteroidUpgrades[upgradeId] || 0;
      ...
      wallet.tokens -= cost;
      wallet.asteroidUpgrades[upgradeId] = level + 1;
      saveData(data);
      return { wallet };
    });
    if (result.error) return res.status(400).json(result);
