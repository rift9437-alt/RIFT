...
const REWARDS = {
  ...
  roguelike_floor: 6,    // per floor reached (deepest floor that run)
  roguelike_kill: 1      // per monster slain that run
};

// Each theme also carries a `bg` color — applied to the page's --bg CSS
// variable so equipping a theme recolors the whole site's background, not
// just borders/text/accents.
const SHOP_ITEMS = {
  neon:   { name: 'Neon Default', cost: 0,   cyan: '#2de2c5', pink: '#ff3d8a', gold: '#ffc857', bg: '#0a0d13' },
  sunset: { name: 'Sunset Drift', cost: 150, cyan: '#ff7a45', pink: '#ff4d6d', gold: '#ffd23f', bg: '#170d0a' },
  toxic:  { name: 'Toxic Lab',    cost: 150, cyan: '#9dff45', pink: '#45ffb0', gold: '#e8ff45', bg: '#0a1309' },
  royal:  { name: 'Royal Velvet', cost: 200, cyan: '#c9a6ff', pink: '#ff8fd1', gold: '#ffd24d', bg: '#120a1c' },
  blood:  { name: 'Blood Moon',   cost: 250, cyan: '#ff5454', pink: '#ff8a8a', gold: '#ffae42', bg: '#170707' },
  arctic: { name: 'Arctic Drift', cost: 200, cyan: '#7dd3ff', pink: '#c3e9ff', gold: '#e0f7ff', bg: '#070f18' },
  vaporwave: { name: 'Vaporwave', cost: 250, cyan: '#5ff0ff', pink: '#ff6ad5', gold: '#c774e8', bg: '#100a1e' }
};
...
const DEFAULT_WALLET = {
  ...
  roguelikeUpgrades: { extraHp: 0, swordDamage: 0, magicPower: 0, moveSpeed: 0 },
  breakerUpgrades: { extraLife: 0, widePaddle: 0, slowBall: 0 }
};
...
// ---------------------------------------------------------------------------
// Void Crawler (roguelike) upgrades — permanent, carry over between runs.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Neon Breaker upgrades
// ---------------------------------------------------------------------------
const BREAKER_UPGRADES = {
  extraLife: {
    name: 'Extra Life',
  ...
  widePaddle: {
    name: 'Wide Paddle',
    desc: '+15% paddle width per level',
    maxLevel: 3,
    costs: [60, 120, 200]
  },
  slowBall: {
    name: 'Steady Hands',
    desc: '-10% ball speed growth per level, easier to control',
    maxLevel: 2,
    costs: [70, 140]
  }
};
...
[2026-07-27] VOID CRAWLER added to the arcade — a top-down roguelike. Move with WASD/arrows, SPACE to swing your sword, F to cast a magic bolt. Every floor's room, monsters, and loot are randomized, a boss waits every 5th floor, and death sends you back to floor 1 — but any tokens and permanent upgrades you bought stick around. Deepest floor reached is tracked on the Leaderboard.
[2026-07-27] SHOP — equipping a color theme now also recolors the site background to match, not just the accent colors.
[2026-07-27] NEON BREAKER — token upgrades added. Spend your tokens on Extra Life, a Wide Paddle, and Steady Hands (slower ball speed growth). Pick your upgrades from the launch screen before you play.
  ...
  USERS.forEach(u => {
    ...
    if (!w || typeof w !== 'object') {
      ...
      if (!w.roguelikeUpgrades || typeof w.roguelikeUpgrades !== 'object') {
        w.roguelikeUpgrades = { extraHp: 0, swordDamage: 0, magicPower: 0, moveSpeed: 0 };
      } else {
        Object.keys(ROGUELIKE_UPGRADES).forEach(id => {
          if (typeof w.roguelikeUpgrades[id] !== 'number') w.roguelikeUpgrades[id] = 0;
        });
      }
      if (!w.breakerUpgrades || typeof w.breakerUpgrades !== 'object') {
        w.breakerUpgrades = { extraLife: 0, widePaddle: 0, slowBall: 0 };
      } else {
        Object.keys(BREAKER_UPGRADES).forEach(id => {
          if (typeof w.breakerUpgrades[id] !== 'number') w.breakerUpgrades[id] = 0;
        });
      }
    }
  });
...
app.post('/api/roguelike/upgrades/purchase', async (req, res) => {
  ...
  try {
    ...
    console.error('Roguelike upgrade purchase failed:', e);
    res.status(500).json({ error: 'Internal error' });
  }
});

app.get('/api/breaker/upgrades', (req, res) => {
  res.json(BREAKER_UPGRADES);
});

app.post('/api/breaker/upgrades/purchase', async (req, res) => {
  const { user, upgradeId } = req.body || {};
  if (!USERS.includes(user)) {
    return res.status(400).json({ error: 'Unknown user' });
  }
  if (!BREAKER_UPGRADES[upgradeId]) {
    return res.status(400).json({ error: 'Unknown upgrade' });
  }
  ...
  try {
    const result = await withWriteLock(async () => {
      const data = await loadData();
      const wallet = data[user].wallet;
      const outcome = purchaseLeveledUpgrade(wallet, BREAKER_UPGRADES, 'breakerUpgrades', upgradeId);
      if (outcome.error) return outcome;
      await saveData(data);
      return outcome;
    });
    ...
    console.error('Breaker upgrade purchase failed:', e);
    res.status(500).json({ error: 'Internal error' });
  }
});
