...
const ACHIEVEMENTS = {
  first_victory:  { name: 'First Victory',        icon: '🥇', desc: 'Win your first game.',                         reward: { tokens: 100 } },
  goals_100:      { name: 'Score 100 Goals',       icon: '⚽', desc: 'Score 100 goals across all Street Soccer matches.', reward: { tokens: 200 } },
  floor_50:       { name: 'Reach Floor 50',        icon: '💀', desc: 'Reach floor 50 in Crypt Crawler.',              reward: { tokens: 300 } },
  tokens_10000:   { name: 'Earn 10,000 Tokens',    icon: '💰', desc: 'Earn 10,000 tokens over your lifetime.',        reward: { tokens: 250 } },
  win_streak_20:  { name: 'Win 20 Games in a Row', icon: '🔥', desc: 'Win 20 competitive games in a row.',            reward: { tokens: 400, title: 'Unstoppable' } },
  bricks_5000:    { name: 'Break 5,000 Bricks',    icon: '🧱', desc: 'Break 5,000 bricks in Neon Breaker.',           reward: { tokens: 200 } },
  races_100:      { name: 'Finish 100 Races',      icon: '🏎', desc: 'Finish 100 races in Apex Loop.',                reward: { tokens: 250 } },
  legendary:      { name: 'Legendary Player',      icon: '👑', desc: 'Unlock every other achievement.',               reward: { tokens: 1000, title: 'Legend', border: 'legendary-border' } }
};
...
const DEFAULT_WALLET = { tokens: 0, owned: ['neon'], equipped: 'neon', companion: null, asteroidUpgrades: { extraLife: 0, turnSpeed: 0, autoTurret: 0 }, wildduelUpgrades: { extraHp: 0, fasterReload: 0, fasterMovement: 0 }, roguelikeUpgrades: { extraHp: 0, swordDamage: 0, magicPower: 0, swiftBoots: 0 }, achievements: [], stats: { bricksBroken: 0, racesFinished: 0, winStreak: 0, bestWinStreak: 0, tokensEarnedLifetime: 0 }, titles: [], borders: [], seasonBadges: [] };
  ...
  USERS.forEach(u => {
    ...
    if (!w || typeof w !== 'object') {
      ...
      if (!w.stats || typeof w.stats !== 'object') {
        w.stats = { bricksBroken: 0, racesFinished: 0, winStreak: 0, bestWinStreak: 0, tokensEarnedLifetime: 0 };
      } else {
        for (const key of ['bricksBroken', 'racesFinished', 'winStreak', 'bestWinStreak', 'tokensEarnedLifetime']) {
...
app.post('/api/wallet/earn', async (req, res) => {
  ...
  try {
    const updated = await withWriteLock(async () => {
      ...
      if (!wallet.stats) wallet.stats = { bricksBroken: 0, racesFinished: 0, winStreak: 0, bestWinStreak: 0, tokensEarnedLifetime: 0 };
      if (amount > 0) wallet.stats.tokensEarnedLifetime += amount;
