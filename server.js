...
const ACHIEVEMENTS = {
  first_victory:  { name: 'First Victory',         icon: '🥇', desc: 'Win your first game.',                                reward: { tokens: 300 } },
  goals_100:      { name: 'Score 100 Goals',        icon: '⚽', desc: 'Score 100 goals across all Street Soccer matches.',   reward: { tokens: 2000 } },
  floor_50:       { name: 'Reach Floor 50',         icon: '💀', desc: 'Reach floor 50 in Crypt Crawler.',                    reward: { tokens: 2500 } },
  tokens_10000:   { name: 'Earn 10,000 Tokens',     icon: '💰', desc: 'Earn 10,000 tokens over your lifetime.',              reward: { tokens: 2000 } },
  ...
  bricks_5000:    { name: 'Break 5,000 Bricks',     icon: '🧱', desc: 'Break 5,000 bricks in Neon Breaker.',                 reward: { tokens: 2000 } },
  races_100:      { name: 'Finish 100 Races',       icon: '🏎', desc: 'Finish 100 races in Apex Loop.',                     reward: { tokens: 2500 } },

  wins_100:       { name: 'Century Club',           icon: '💯', desc: 'Win 100 games total across the whole arcade.',       reward: { tokens: 3000 } },
  ...
  tokens_50000:   { name: 'High Roller',            icon: '🏦', desc: 'Earn 50,000 tokens over your lifetime.',             reward: { tokens: 4000 } },
  bricks_20000:   { name: 'Wrecking Ball',          icon: '💥', desc: 'Break 20,000 bricks in Neon Breaker.',               reward: { tokens: 4000 } },
...
const WIN_REASONS = ['soccer_win', 'racing_win', 'tank_win', 'wildduel_win', 'sumo_win'];
const LOSS_REASONS = ['soccer_loss', 'racing_loss', 'tank_loss', 'wildduel_loss', 'sumo_loss'];
...
function checkAchievements(data, user) {
  ...
  if (!wallet.stats) wallet.stats = { bricksBroken: 0, racesFinished: 0, winStreak: 0, bestWinStreak: 0, tokensEarnedLifetime: 0 };
...
const SEASONS = {
  1: { name: 'Season 1', theme: 'Beach', badge: '🏖️', limitedThemeId: 'beachwave', blurb: 'Sun, surf, and a very limited-time Beach Wave theme in the shop.' }
};
...
const DEFAULT_WALLET = { tokens: 0, owned: ['neon'], equipped: 'neon', companion: null, asteroidUpgrades: { extraLife: 0, turnSpeed: 0, autoTurret: 0 }, wildduelUpgrades: { extraHp: 0, fasterReload: 0, fasterMovement: 0 }, roguelikeUpgrades: { extraHp: 0, swordDamage: 0, magicPower: 0, swiftBoots: 0 }, achievements: [], stats: { bricksBroken: 0, racesFinished: 0, winStreak: 0, bestWinStreak: 0, tokensEarnedLifetime: 0, secondsPlayed: 0 }, titles: [], borders: [], seasonBadges: [], xp: 0, level: 1, avatar: '🙂', banner: 'default', friends: [], animatedName: false, createdAt: null, gamePlays: {} };
  ...
  USERS.forEach(u => {
    ...
    if (!w || typeof w !== 'object') {
      ...
      if (!w.stats || typeof w.stats !== 'object') {
        w.stats = { bricksBroken: 0, racesFinished: 0, winStreak: 0, bestWinStreak: 0, tokensEarnedLifetime: 0, secondsPlayed: 0 };
      } else {
        for (const key of ['bricksBroken', 'racesFinished', 'winStreak', 'bestWinStreak', 'tokensEarnedLifetime', 'secondsPlayed']) {
      ...
      if (!AVATARS.includes(w.avatar)) w.avatar = '🙂';
      if (!BANNERS.includes(w.banner)) w.banner = 'default';
...
app.get('/api/profile/options', (req, res) => {
  res.json({ avatars: AVATARS, banners: BANNERS });
});
...
app.post('/api/profile/customize', async (req, res) => {
  ...
  if (banner !== undefined && !BANNERS.includes(banner)) {
  ...
  try {
    const updated = await withWriteLock(async () => {
      const data = await loadData();
      const wallet = data[user].wallet;
      if (avatar !== undefined) wallet.avatar = avatar;
      if (banner !== undefined) wallet.banner = banner;
      await saveData(data);
      return data[user].wallet;
    });
    res.json(updated);
...
app.get('/api/profile', async (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      achievements: { unlocked: (wallet.achievements || []).length, total: Object.keys(ACHIEVEMENTS).length },
